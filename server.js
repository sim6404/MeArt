...
process.env.PYTHONPATH = [
  process.env.PYTHONPATH || '',
  '/opt/render/.local/lib/python3.*/site-packages'
].filter(Boolean).join(':');
...
// CORS 설정 (Render/커스텀 도메인 대응)
function getAllowedOrigins() {
    const defaults = [
        'http://localhost:9000',
        'http://127.0.0.1:9000',
        'https://meart-app.onrender.com',
        'null'
    ];
...
// 갤러리 API - 업로드된 이미지 목록 조회 (공개 API)
app.get('/api/gallery', (req, res) => {
    try {
        console.log('🎨 갤러리 API 호출됨');
        ...
        const images = files
            ...
            .map(file => {
                ...
                if (file.includes('_brush_')) {
                    type = 'brush';
                    const emotionMatch = file.match(/_brush_(\w+)_/);
                    if (emotionMatch) emotion = emotionMatch[1];
                } else if (file.includes('_preview_')) {
                    type = 'preview';
                } else if (file.includes('_nobg')) {
                    type = 'nobg';
                }
                
                // 제목 생성
                let title = file.replace(path.extname(file), '');
                if (type === 'brush') title = `🎨 브러시 아트 (${emotion})`;
                else if (type === 'preview') title = `🖼️ AI 아트 결과`;
                else if (type === 'nobg') title = `✂️ 배경 제거`;
                else title = `📸 원본 이미지`;
                ...
                return {
                    ...
                    url: `/uploads/${file}`,
                    title: title,
                    emotion: emotion !== 'unknown' ? emotion : null,
                    type: type,
                    size: stats.size,
                    date: stats.mtime.toLocaleDateString('ko-KR'),
...
async function validateImage(filePath) {
    try {
        ...
        // python → py로 변경
        // Windows의 'py'가 아닌 리눅스/컨테이너 환경 호환
        const result = execSync(`python3 check_alpha_file.py "${filePath}"`).toString();
...
app.post('/api/apply-brush-effect', async (req, res) => {
    try {
        ...
        if (!fs.existsSync(nobgAbsPath)) {
            ...
            // 원본 파일을 찾지 못한 경우, 가장 최근의 이미지 파일을 사용
            if (!originalFile || !fs.existsSync(originalFile)) {
                console.log('🔍 원본 파일을 찾지 못했습니다. 최근 이미지 파일을 찾습니다.');
                try {
                    const files = fs.readdirSync(uploadsDir);
                    const imageFiles = files.filter(file => {
                        const ext = path.extname(file).toLowerCase();
                        return ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp'].includes(ext) &&
                               !file.includes('_nobg') && !file.includes('_preview') && !file.includes('_brush');
                    });
                    ...
                    if (imageFiles.length > 0) {
                        ...
                        originalFile = path.join(uploadsDir, sortedFiles[0]);
                        console.log('🔄 최근 이미지 파일을 사용합니다:', originalFile);
                    }
                } catch (error) {
                    console.error('❌ 최근 이미지 파일 찾기 실패:', error);
                }
            }
        ...
        try {
            await runPythonScript('brush_effect_minimal.py', [nobgAbsPath, brushPath]);
        } catch (pythonError) {
...
async function processImagePipeline({ inputPath, outputPath, emotion, backgroundPath }) {
    ...
    await runPythonScript('brush_effect_minimal.py', [nobgPath, brushPath]);
...
app.post('/api/brush-effect-only', upload.single('image'), async (req, res) => {
    try {
        ...
        await runPythonScript('brush_effect_minimal.py', [req.file.path, brushedPath]);
...
// 감정 게임 메시지 생성 API (Claude API 프록시)
app.post('/api/generate-emotion-message', async (req, res) => {
    try {
        ...
        if (!ANTHROPIC_API_KEY) {
            console.warn('ANTHROPIC_API_KEY가 설정되지 않았습니다.');
            return res.status(500).json({ 
                error: 'API 키가 설정되지 않았습니다.',
                fallbackMessage: '이 순간이 지나가면 더 밝은 내일이 기다리고 있을 거예요.'
            });
        }
        ...
        const message = response.data.content && response.data.content[0] && response.data.content[0].text 
